...
import { DrawerItem, DrawerItemList, useDrawerStatus } from '@react-navigation/drawer';
...
const ListModifier = () => {
  const drawerStatus: DrawerStatus = useDrawerStatus();
  const [isExNameInputVisible, setIsExNameInputVisible] = useState(false);
  const [exNameInput, setExNameInput] = useState('');
  useEffect(() => {
    if (drawerStatus === 'closed') {
      setExNameInput('');
      setIsExNameInputVisible(false);
    }
  }, [drawerStatus]);

  return (
    <>
      {isExNameInputVisible && (
        <TextInput
          className="rounded-xl border-2 border-black bg-white px-2"
          value={exNameInput}
          onChangeText={newText => setExNameInput(newText)}
      ...
      <View className="flex-row gap-2">
        <Pressable
          className="flex-1 items-center rounded-md bg-blue-600 p-2 hover:bg-blue-700 active:opacity-20"
          onPress={() => setIsExNameInputVisible(true)}>
          <AntDesign name="pluscircle" size={20} color="white" />
        </Pressable>

        <Pressable
          className="flex-1 items-center rounded-md bg-gray-500 p-2 hover:bg-gray-300 active:opacity-20"
          onPress={() => console.log('Edit Exercise button pressed')}>
